"""Connection lifecycle for Clover POS Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from clover_pos_client import CloverPOSClient
...
_SECRET = "clover_pos_connections"
...
@chat.function(
    "connect_clover_pos",
    "Connect Clover POS account via credentials.",
    action_type="write",
    chain_callable=True,
    event="clover-pos-connector.connect_clover_pos",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_clover_pos(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Clover POS Connector."""
    client = CloverPOSClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Clover POS Account",
...
@chat.function(
    "list_connections",
    "List connected Clover POS accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_clover_pos",
    "Disconnect Clover POS account.",
    action_type="write",
    chain_callable=True,
    event="clover-pos-connector.disconnect_clover_pos",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_clover_pos(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
