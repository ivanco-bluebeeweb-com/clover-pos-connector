"""Connection management for Clover POS Connector."""
...
from clover_pos_connector_client import CloverPOSClient

_SECRET = "clover_pos_connector_connections"
...
async def resolve_client(ctx, connection_id: str = "") -> CloverPOSClient:
    conns = await _load_conns(ctx)
    if not conns:
        raise ValueError("No Clover POS connections configured. Use connect_clover_pos_connector first.")
    conn = conns[0]
    if connection_id:
        for c in conns:
...
async def connect_clover_pos_connector(params: ConnectParams, ctx) -> ActionResult:
    ...
    if res.get("status") == "error":
        return ActionResult.error(f"Failed to connect to Clover POS: {res.get('error')}")
    conns = await _load_conns(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    rec = {
        "id": cid,
        "label": params.label or "Primary Clover POS",
    ...
    return ActionResult.ok(rec, summary=f"Connected Clover POS ({rec['label']}).")
...
async def list_connections(params: NoParams, ctx) -> ActionResult:
    ...
    items = [{
        ...
        "base_url": c.get("base_url", "https://api.clover.com/v3"),
...
async def disconnect_clover_pos_connector(params: ConnectionIdParams, ctx) -> ActionResult:
    ...
    if params.connection_id:
        ...
        conns.clear()
    await _save_conns(ctx, conns)
    return ActionResult.ok({"success": True, "message": "Disconnected successfully."}, summary="Disconnected Clover POS connection.")
