"""Resource handlers for Clover POS Connector."""
...
@chat.function("list_orders", "List orders in Clover POS.", action_type="read", chain_callable=True, event="clover-pos-connector.list_orders", effects=["read:orders"], data_model=OrderList)
...
@chat.function("get_order", "Get details of one Order in Clover POS.", action_type="read", chain_callable=True, event="clover-pos-connector.get_order", effects=["read:order"], data_model=OrderRecord)
...
@chat.function("audit_order_health", "Audit health of Clover POS orders and connectivity.", action_type="read", chain_callable=True, event="clover-pos-connector.audit_order_health", effects=["read:audit"], data_model=AuditHealthReport)
async def audit_order_health(params: ConnectionIdParams, ctx) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.ok({
            ...
            "summary": f"Clover POS healthy. Sampled {len(items)} orders."
        }, summary=f"Clover POS health check passed with {len(items)} orders.")
    except Exception as e:
        return ActionResult.error(f"Error auditing Clover POS health: {e}")
